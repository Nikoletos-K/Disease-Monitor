...
Patient * createPatient(char * id,char * firstName,char * lastName ,char * disease,char * enterORexit,int age){

	Patient * p = malloc(sizeof(Patient));
	p->id = strdup(id);
	p->firstName = strdup(firstName);
	p->lastName = strdup(lastName);
	p->enterORexit = strdup(enterORexit);
...
void print_patientInfo(void * p){
	printf("\t%s %s %s %s %s %d \n",((Patient *) p)->id,((Patient *) p)->firstName,((Patient *) p)->lastName,((Patient *) p)->disease,((Patient *) p)->enterORexit,((Patient *) p)->age);
}
